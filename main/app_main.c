...
void wifi_sniffer_cb(void *recv_buf, wifi_promiscuous_pkt_type_t type)
{
    ...
    s_device_info_num++;
    printf("%5d\. MAC: 0x%02X.0x%02X.0x%02X.0x%02X.0x%02X.0x%02X, TIME: %d, RSSI = %d\r\n", s_device_info_num, station_info->bssid[0], station_info->bssid[1], station_info->bssid[2], station_info->bssid[3], station_info->bssid[4], station_info->bssid[5], station_info->timestamp, station_info->rssi);
}
